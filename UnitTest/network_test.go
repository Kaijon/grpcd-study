...
func TestUpdateIPv4(t *testing.T) {
	...
	if AppConfig.Network.IPv4 != newIPv4 {
        t.Errorf("Expcted IPv4 update to: %s, but get %s", newIPv4, AppConfig.Network.IPv4)
	}
}
...
func TestGetIPv4(t *testing.T) {
	...
	if response.IPv4 != AppConfig.Network.IPv4 {
        t.Errorf("Expected IPv4: %s, but get %s", AppConfig.Network.IPv4, response.IPv4)
	}
}
...
func TestGetIPv6(t *testing.T) {
	...
	if response.IPv6 != AppConfig.Network.IPv6 {
        t.Errorf("Expcted IPv6: %s, but get %s", AppConfig.Network.IPv6, response.IPv6)
	}
}
/*
...
func TestGetAllNetworkInfo(t *testing.T) {
	...
	if response.IPv4 != AppConfig.Network.IPv4 || response.IPv6 != AppConfig.Network.IPv6 {
		t.Errorf("Expected results are not equal to response, IPv4 Expcted/Response: %s/%s, IPv6 Expcted/Exponse: %s/%s", AppConfig.Network.IPv4, response.IPv4, AppConfig.Network.IPv6, response.IPv6)
	}
}
