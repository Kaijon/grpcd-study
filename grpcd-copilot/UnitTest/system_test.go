...
func TestDeviceInfoServer_SetTime(t *testing.T) {
	...
	cfg.ReadConfig(func(current cfg.Config) {
		if current.System.Time != currentTimeStr {
			t.Errorf("Expcted Time update to: %s, but get %s", currentTimeStr, current.System.Time)
		}
	})
}
...
func TestDeviceInfoServer_GetAllSystemInfo(t *testing.T) {
	...
	var expected *pb.GetAllSystemInfoResponse
	cfg.ReadConfig(func(current cfg.Config) {
		expected = &pb.GetAllSystemInfoResponse{
			FWVersion:  current.System.FWVersion,
			Time:       currentTimeStr,
			SerialNo:   current.System.SerialNo,
			SKUName:    current.System.SKUName,
			DeviceName: current.System.DeviceName,
			MAC:        current.System.MAC,
		}
	})
